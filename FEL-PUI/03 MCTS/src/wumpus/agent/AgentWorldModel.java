...
//	}
	...
	public static Set<Transition> getTransitions(WorldState state, Action action) {
		...
		CellContent currentContent = state.getMap()[state.getX()][state.getY()];
		
		if (currentContent == CellContent.EMPTY) {
...
//	}
    ...
    private static WorldState moveAgent(WorldState state, Action action) {
		...
		return new WorldState(actualTarget, state.getWumpuses(), state.getMap(), state.getActionsLeft() - 1);
	}
