...
public class RobotEmilCreator {
	...
	final static int SIMULATION_STEP_DELAY = 1;
    ...
    private void create(RobotEmilAgent robot) {
		...
		for (int j = 0; j < 5; j++) {
    		for (int i=0; i<RUNS; i++) {
	    		...
	    		boolean success = simulate(robot, new Random(i), SIMULATION_STEP_DELAY, i);
	    		if (success) {
	    			successCount++;
	    		}
    ...
    private boolean simulate(RobotEmilAgent agent, Random rnd, int visualizationDelay, int iteration) {
    	x = 0;
    	y = 0;

    	for (int i=0; i<STEPS; i++) {
    		try {
				Action action = agent.nextStep(x, y, map, iteration);
	...
	public static void main(String[] args) {
    	new RobotEmilCreator().create(new RobotEmilDijkstraAgent());
    }
}
