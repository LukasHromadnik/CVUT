...
public:
	...
	EvolutionSolution * solve(Knapsack *knapsack);
};
...
EvolutionSolution * Evolution::solve(Knapsack *knapsack) {
	// Initial population
	Population *population = new Population(m_PopulationCount, knapsack->m_Items.size(), knapsack);

	// Print the first generation value
	// cout << population->value() << endl;
	...
	while (numberOfGenerations < m_MaxNumberOfGenerations) {
		...
		// Print current population value
		cout << population->value() << endl;
