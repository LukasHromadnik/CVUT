...
int main(int argc, char const *argv[])
{
	...
	string outputSource = filename + "_dec" + appendix;
	...
	EVP_DecryptInit(&ctx, (method == ECB) ? EVP_des_ecb() : EVP_des_cbc(), key, initialVector);
	...
	while (inFile.get(load)) {
		...
		// Sifrovani
		EVP_DecryptUpdate(&ctx, encryptedBody, &encryptedBodyLength, letter, 1);
	...
	EVP_DecryptFinal(&ctx, encryptedBody, &encryptedBodyLength);
